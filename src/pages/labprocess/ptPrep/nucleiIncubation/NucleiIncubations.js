...
import { register, updateStatus } from '../../../../actions/nucleiIncubation';
...
import NucleiIncubationsActions from './NucleiIncubationsActions'
import PtPrepSDMenu from '../../../../components/labprocess/ptPrep/ptPrepSDMenu'



export default function NucleiIncubations() {

  const {
    state: { nucleiIncubations },
    dispatch,
  } = useValue();
  ...
  const [rows, setRows] = useState(nucleiIncubations);
  const [rowModesModel, setRowModesModel] = useState({});

  useEffect(() => {

    setRows(nucleiIncubations)
  ...
  const columns = useMemo(
    () =>  [
    {
      ...
      renderCell: (params) => (
        <NucleiIncubationsActions {...{ params, rows, setRows, rowModesModel, setRowModesModel }} />
      ),
    },
    { field: 'assay_batch_name', headerName: 'Batch Name', flex: 1, },
    { field: 'assay_tubeNum', headerName: 'Tube #', flex: 1, editable: true },
